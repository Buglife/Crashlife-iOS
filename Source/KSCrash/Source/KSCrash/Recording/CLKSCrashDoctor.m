...
@interface  CLKSCrashDoctorParam: NSObject
...
@implementation CLKSCrashDoctorParam
...
@interface CLKSCrashDoctorFunctionCall: NSObject
...
@implementation CLKSCrashDoctorFunctionCall
...
- (NSString*) descriptionForObjCCall
{
    ...
    CLKSCrashDoctorParam* receiverParam = [self.params objectAtIndex:0];
    ...
    CLKSCrashDoctorParam* selectorParam = [self.params objectAtIndex:1];
    if(![selectorParam.type isEqualToString:@CLKSCrashMemType_String])
    {
        return nil;
    }
    ...
    for(int paramNum = 0; paramNum < paramCount; paramNum++)
    {
        ...
        if(paramNum < 2)
        {
            CLKSCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)paramNum + 2];
            if(param.value != nil)
            {
                if([param.type isEqualToString:@CLKSCrashMemType_String])
                {
...
- (NSString*) descriptionWithParamCount:(int) paramCount
{
    ...
    {
        CLKSCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)i];
...
@implementation CLKSCrashDoctor
...
{
    return [[self crashReport:report] objectForKey:@CLKSCrashField_Error];
}
...
- (NSDictionary*) crashedThreadReport:(NSDictionary*) report
{
    ...
    NSDictionary* crashedThread = [crashReport objectForKey:@CLKSCrashField_CrashedThread];
    if(crashedThread != nil)
    {
        return crashedThread;
    }

    for(NSDictionary* thread in [crashReport objectForKey:@CLKSCrashField_Threads])
    {
        if([[thread objectForKey:@CLKSCrashField_Crashed] boolValue])
        {
            return thread;
        }
    }
...
{
    NSDictionary* backtrace = [threadReport objectForKey:@CLKSCrashField_Backtrace];
    return [backtrace objectForKey:@CLKSCrashField_Contents];
}

- (NSDictionary*) basicRegistersFromThreadReport:(NSDictionary*) threadReport
{
    NSDictionary* registers = [threadReport objectForKey:@CLKSCrashField_Registers];
    NSDictionary* basic = [registers objectForKey:@CLKSCrashField_Basic];
    return basic;
}
...
- (NSDictionary*) lastInAppStackEntry:(NSDictionary*) report
{
    ...
    {
        NSString* objectName = [entry objectForKey:@CLKSCrashField_ObjectName];
...
{
    NSDictionary* machError = [errorReport objectForKey:@CLKSCrashField_Mach];
    if(machError != nil)
    {
        NSString* exceptionName = [machError objectForKey:@CLKSCrashField_ExceptionName];
        return [exceptionName isEqualToString:@"EXC_BAD_ACCESS"];
    }
    NSDictionary* signal = [errorReport objectForKey:@CLKSCrashField_Signal];
    NSString* sigName = [signal objectForKey:@CLKSCrashField_Name];
...
{
    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSArray* notableAddresses = [crashedThread objectForKey:@CLKSCrashField_NotableAddresses];
    for(NSDictionary* address in [notableAddresses objectEnumerator])
    {
        NSString* type = [address objectForKey:@CLKSCrashField_Type];
        if([type isEqualToString:@"string"])
        {
            NSString* value = [address objectForKey:@CLKSCrashField_Value];
    ...
    {
        NSString* objectName = [entry objectForKey:@CLKSCrashField_ObjectName];
        NSString* symbolName = [entry objectForKey:@CLKSCrashField_SymbolName];
...
- (CLKSCrashDoctorFunctionCall*) lastFunctionCall:(NSDictionary*) report
{
    CLKSCrashDoctorFunctionCall* function = [[CLKSCrashDoctorFunctionCall alloc] init];
    NSDictionary* lastStackEntry = [self lastStackEntry:report];
    function.name = [lastStackEntry objectForKey:@CLKSCrashField_SymbolName];
    ...
    {
        CLKSCrashDoctorParam* param = [[CLKSCrashDoctorParam alloc] init];
        ...
        {
            param.type = [notableAddress objectForKey:@CLKSCrashField_Type];
            NSString* className = [notableAddress objectForKey:@CLKSCrashField_Class];
            NSString* previousClass = [notableAddress objectForKey:@CLKSCrashField_LastDeallocObject];
            NSString* value = [notableAddress objectForKey:@CLKSCrashField_Value];

            if([param.type isEqualToString:@CLKSCrashMemType_String])
            {
                param.value = value;
            }
            else if([param.type isEqualToString:@CLKSCrashMemType_Object])
            {
                param.className = className;
                param.isInstance = YES;
            }
            else if([param.type isEqualToString:@CLKSCrashMemType_Class])
            {
                param.className = className;
                param.isInstance = NO;
            }
...
- (NSString*) zombieCall:(CLKSCrashDoctorFunctionCall*) functionCall
{
...
{
    NSDictionary* stack = [crashedThreadReport objectForKey:@CLKSCrashField_Stack];
    return [[stack objectForKey:@CLKSCrashField_Overflow] boolValue];
}

- (BOOL) isDeadlock:(NSDictionary*) report
{
    NSDictionary* errorReport = [self errorReport:report];
    NSString* crashType = [errorReport objectForKey:@CLKSCrashField_Type];
    return [@CLKSCrashExcType_Deadlock isEqualToString:crashType];
}
...
- (NSString*) diagnoseCrash:(NSDictionary*) report
{
    @try
    {
        NSString* lastFunctionName = [[self lastInAppStackEntry:report] objectForKey:@CLKSCrashField_SymbolName];
        ...
        NSString* crashType = [errorReport objectForKey:@CLKSCrashField_Type];
        if([crashType isEqualToString:@CLKSCrashExcType_NSException])
        {
            NSDictionary* exception = [errorReport objectForKey:@CLKSCrashField_NSException];
            NSString* name = [exception objectForKey:@CLKSCrashField_Name];
            NSString* reason = [exception objectForKey:@CLKSCrashField_Reason]? [exception objectForKey:@CLKSCrashField_Reason]:[errorReport objectForKey:@CLKSCrashField_Reason];
        ...
        CLKSCrashDoctorFunctionCall* functionCall = [self lastFunctionCall:report];
        ...
        {
            uintptr_t address = (uintptr_t)[[errorReport objectForKey:@CLKSCrashField_Address] unsignedLongLongValue];
            if(address == 0)
            {
    ...
    @catch (NSException* e)
    {
        ...
        if(symbols)
        {
            return [NSString stringWithFormat:@"No diagnosis due to exception %@:\n%@\nPlease file a bug report to the Crashlife project.", e, symbols];
        }
        return [NSString stringWithFormat:@"No diagnosis due to exception %@\nPlease file a bug report to the Crashlife project.", e];
    }
}
